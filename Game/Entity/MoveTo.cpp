...
{
        if (pos_character.first < 0 || pos_character.second < 0 || pos_character.first >= map.size())
                return {};
...
std::pair<int, int> MoveTo::try_move_left(std::vector<std::vector<std::vector<std::shared_ptr<entities::Entity>>>> &map,
				  std::pair<int, int> pos_character)
{
	if (pos_character.first < 0 || pos_character.second < 0 || pos_character.second >= map[pos_character.first].size())
		return {};
	for (auto &entity : map[pos_character.first][pos_character.second + 1]) {
		if (std::static_pointer_cast<Walls>(entity) != nullptr)
			return {};
